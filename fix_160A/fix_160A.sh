...
VERBOSE=0                   # 0=silent, 1=basic logging, 2=verbose (includes DFS status blocks)
...
wait_for_cac() {
    ...
    log 1 "[INFO] Polling for CAC completion (poll=${CAC_POLL}s, timeout=${CAC_TIMEOUT}s)..."
    ...
    while [ "$elapsed" -lt "$CAC_TIMEOUT" ]; do
        ...
        log 1 "[INFO] CAC poll ${elapsed}s: $move_status"
        if [ "$VERBOSE" -ge 2 ]; then
            log 2 "[DFS_STATUS:POLL-${elapsed}s]"
            echo "$status" | while IFS= read -r line; do
                echo "$(date):   $line" >> "$LOG_FILE"
            done
        fi
        if echo "$move_status" | grep -q "move status=-1"; then
            log 1 "[INFO] CAC complete after ${elapsed}s."
            return 0
        fi
    done

    log 1 "[NOTICE] CAC timed out after ${elapsed}s. Holding until next cron run."
    return 1
}
